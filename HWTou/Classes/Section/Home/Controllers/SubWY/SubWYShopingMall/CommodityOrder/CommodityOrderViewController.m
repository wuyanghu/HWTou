//
//  CommodityOrderViewController.m
//  HWTou
//
...
@interface CommodityOrderViewController ()<VTMagicViewDataSource,VTMagicViewDelegate,CommodityOrderDetailDelegate>
@property (nonatomic, strong) VTMagicController *magicController;
@property (nonatomic,strong) NSMutableArray * listTitle;
@end

@implementation CommodityOrderViewController
...
- (void)viewDidLoad {
    ...
    if (_isPay) {
        UIBarButtonItem *leftItem = [UIBarButtonItem itemWithImageName:@"hfzk_btn_back" hltImageName:@"hfzk_btn_back"
...
- (void)commodityOrderGoPayPushAction:(GetGoodsOrderListModel *)model{
    PayWayViewController * detail2VC = [[PayWayViewController alloc] initWithNibName:nil bundle:nil];
    [detail2VC setGetGoodsOrderListModel:model];
    detail2VC.type = PayWayTypeOrder;
...
- (void)orderPushDetailVC:(GetGoodsOrderListModel *)model{
    ...
    CommodityOrderDetail1ViewController * cartVC = [[CommodityOrderDetail1ViewController alloc] initWithNibName:nil bundle:nil];
...
- (UIViewController *)magicView:(VTMagicView *)magicView viewControllerAtPage:(NSUInteger)pageIndex
{
    ...
    CommodityOrderDetailViewController * contentVC = [magicView dequeueReusablePageWithIdentifier:recomId];

    if (!contentVC) {
        contentVC = [[CommodityOrderDetailViewController alloc] init];
