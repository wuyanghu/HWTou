...
@interface EditCoverViewController ()<EditCoverViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,TZImagePickerControllerDelegate>
{
    ActionSheetTag actionSheetTag;
}
@property (nonatomic,strong) EditCoverView * editDataView;
@end
...
- (void)saveData:(SaveUserDataParam *)saveUserDataParam{
    [_editDelegate selectImgArr:self.editDataView.dataArr];
...
- (void)singleImgUploadWithParam:(UIImage *)image{
    if (actionSheetTag == modifyImageTag) {//修改头像直接替换第一个
        [self.editDataView replaceImage:image];
    }else if (actionSheetTag == selectImageTag){
        [self.editDataView addImage:@[image]];
    }
}

// 图片批量上传
- (void)batchImgUploadWithParam:(NSArray<UIImage *> *)images{
    [self.editDataView addImage:images];
}
...
- (EditCoverView *)editDataView{
    if (!_editDataView) {
        _editDataView = [[EditCoverView alloc] init];
