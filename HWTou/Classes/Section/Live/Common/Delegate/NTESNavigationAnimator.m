...
- (void)pushAnimation:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    {
        frame = CGRectOffset(navigationController.view.frame, 0, navigationController.navigationBar.ntesBottom);
    }
    if ((toViewController.edgesForExtendedLayout & UIRectEdgeBottom) == 0) {
        CGRect slice     = CGRectZero;
        CGRect remainder = CGRectZero;
        CGRectDivide(frame, &slice, &remainder, tabbarController.tabBar.ntesHeight, CGRectMaxYEdge);
        frame = remainder;
    }
    ...
    CGFloat width  = containerView.ntesWidth;
    toViewController.view.ntesLeft = width;
    
    [self callAnimationWillStart];
    CGFloat duration = [self transitionDuration:transitionContext];
    
    [UIView animateWithDuration:duration animations:^{
        fromViewController.view.ntesRight = width * 0.5;
        toViewController.view.ntesRight  = width;
...
- (void)popAnimation:(id<UIViewControllerContextTransitioning>)transitionContext
{
    UIViewController *toViewController = [transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    UIViewController *fromViewController = [transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    CGFloat snapshootHeight = [UIApplication sharedApplication].statusBarFrame.size.height + fromViewController.navigationController.navigationBar.ntesHeight;
    
    UIView          *fakeBar = [fromViewController.navigationController.view
                        resizableSnapshotViewFromRect:CGRectMake(0, 0,fromViewController.view.ntesWidth, snapshootHeight) afterScreenUpdates:NO withCapInsets:UIEdgeInsetsZero];
    ...
    toViewController.view.ntesRight = width * 0.5;
    tabbar.ntesRight = width * 0.5;
    
    [self callAnimationWillStart];
    CGFloat duration = [self transitionDuration:transitionContext];
    [UIView animateWithDuration:duration animations:^{
        fromViewController.view.ntesLeft = width;
        toViewController.view.ntesRight  = width;
        tabbar.ntesRight = width;
        fakeBar.alpha = 0.0;
    } completion:^(BOOL finished) {
