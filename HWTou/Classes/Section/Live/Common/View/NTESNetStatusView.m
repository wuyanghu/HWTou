...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    CGFloat width = self.statusTextLabel.ntesWidth + statusAndIconGap + self.statusImageView.ntesWidth;
    width = width < self.tipLabel.ntesWidth? : self.tipLabel.ntesWidth;
    
    CGFloat statusAndTipGap = 8.f;
    CGFloat height = self.statusTextLabel.ntesHeight + statusAndTipGap + self.tipLabel.ntesHeight;
...
- (void)layoutSubviews
{
    ...
    CGFloat width = self.statusTextLabel.ntesWidth + statusAndIconGap + self.statusImageView.ntesWidth;
    self.statusTextLabel.ntesLeft   = (self.ntesWidth - width) * .5f;
    self.statusImageView.ntesRight  = (self.ntesWidth + width) * .5f;
    self.statusImageView.ntesCenterY = self.statusTextLabel.ntesCenterY;
    
    self.tipLabel.ntesCenterX = self.ntesWidth * 5.f;
    self.tipLabel.ntesBottom  = self.ntesHeight;
}
