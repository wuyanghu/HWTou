...
{
    if (!self.ntesWidth || !self.ntesHeight) {
        return;
    }
    ...
    {
        //ScaleAspectFill模式
        CGPoint center   = CGPointMake(self.ntesWidth * .5f, self.ntesHeight * .5f);
        //哪个小按哪个缩
        CGFloat scaleW   = image.size.width  / self.ntesWidth;
        CGFloat scaleH   = image.size.height / self.ntesHeight;
