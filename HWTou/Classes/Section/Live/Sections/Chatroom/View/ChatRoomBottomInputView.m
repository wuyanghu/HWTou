...
#import "ChatRoomBottomInputView.h"
#import "SZTextView.h"
...
@interface ChatRoomBottomInputView () <UIImagePickerControllerDelegate, UINavigationControllerDelegate, TZImagePickerControllerDelegate, InputLocationSelectDelegate, ReplyRecordBtnDelegate, UITextViewDelegate>
{
...
@implementation ChatRoomBottomInputView
...
- (void)cancelBtnAction {
    ...
    if (self.ChatRoomBottomInputViewDelegate) {
        [self.ChatRoomBottomInputViewDelegate cancelBtnAction];
    }
}
...
- (void)publishBtnAction {
    ...
    if (self.ChatRoomBottomInputViewDelegate) {
        ...
        if (_replyFlag == 1) {
            ...
            [self.ChatRoomBottomInputViewDelegate publishBtnAction:@"" location:self.selectPOIName data:@"语音路径" replyFlag:1 commentModel:_model];
            [self resetInputView];
            ...
            [ComImageUpload batchWithImages:self.selectImagesArr success:^(NSArray<NSString *> *url) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [self.ChatRoomBottomInputViewDelegate publishBtnAction:self.inputTextView.text location:self.selectPOIName data:bmgs replyFlag:2 commentModel:_model];
                    [self resetInputView];
                    [self cancelBtnAction];
                });
            ...
            [VoiceUpload uploadVideoWithVideoPath:self.selectVideoPath title:@"评论视频" tags:@"mp4" videoSize:data.length lat:0 lng:0 successHandle:^(NSString *vidString) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self.ChatRoomBottomInputViewDelegate publishBtnAction:self.inputTextView.text location:self.selectPOIName data:vidString replyFlag:3 commentModel:_model];
                    [self resetInputView];
                    [self cancelBtnAction];
                });
            ...
            //flag == 0
            [self.ChatRoomBottomInputViewDelegate publishBtnAction:self.inputTextView.text location:self.selectPOIName data:@"" replyFlag:0 commentModel:_model];
            [self resetInputView];
            [self cancelBtnAction];
        }
    }
...
- (void)buttonAudioRecorder:(ReplyRecordBtn *)audioRecorder didFinishRcordWithAudioInfo:(NSDictionary *)audioInfo sendFlag:(BOOL)flag{
    ...
    if (flag) {
        ...
        [VoiceUpload uploadVoiceWithVoicepath:voicePath title:fileName tags:@"aac" voiceSize:data.length lat:0 lng:0 successHandle:^(NSString *vidString) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self.ChatRoomBottomInputViewDelegate) {
                    [self.ChatRoomBottomInputViewDelegate publishBtnAction:@"" location:self.selectPOIName data:vidString replyFlag:1 commentModel:_model];
                }
                [self resetInputView];
                [self cancelBtnAction];
            });
...
- (SZTextView *)inputTextView {
    if (!_inputTextView) {
        ...
        _inputTextView.placeholder = @"优质评论会被置顶哦";
