           ...
           inView:(UIView *)view
{
    switch (item.itemType) {
        ...
        case NIMKitRobotTemplateItemTypeImage:
        {
            ...
            imageView.ntesSize = CGSizeMake(item.width.floatValue, item.height.floatValue);
            imageView.image = nil;
            if (item.url.length)
            {
...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    for (UIView *view in self.subviews) {
        ...
        width = MAX(width, view.ntesWidth);
        height += view.ntesHeight;
...
- (void)layoutSubviews
{
    ...
    for (UIView *view in self.subviews)
    {
        ...
        if (view == self.continueButton)
        {
            ...
            self.continueButton.ntesRight  = self.ntesWidth - rightMargin;
            self.continueButton.ntesBottom = self.ntesHeight;
            continue;
        }
        view.ntesLeft = self.model.contentViewInsets.left;
        view.ntesTop = top;
        top = view.ntesBottom + [NTESSessionRobotContentView itemSpacing];
    }
}
...
{
    CGFloat width = superView.ntesWidth - insets.left - insets.right;
    
    for (UIView *subView in superView.subviews)
    {
        if (subView.ntesHeight == 0)
        {
            if ([subView isKindOfClass:[M80AttributedLabel class]])
            {
                M80AttributedLabel *label = (M80AttributedLabel *)subView;
                CGSize size = [label sizeThatFits:CGSizeMake(width, CGFLOAT_MAX)];
                label.ntesSize = CGSizeMake(size.width, size.height);
            ...
            else if ([subView isKindOfClass:[UIImageView class]])
            {
                ...
                imageView.ntesSize = CGSizeMake(defaultImageWidth, defaultImageHeight);
...
- (void)layoutSubviews
{
    ...
    for (UIView *view in self.subviews)
    {
        view.ntesCenterX = self.ntesWidth * .5;
        view.ntesTop = top;
        top = view.ntesBottom + [self itemSpacing];
    }
}
