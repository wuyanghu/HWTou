...
#define SETMUSICDICT @"setmusicdict"//配乐

double OnedayTimeIntervalValue2 = 24*60*60;  //一天的秒数
@implementation NTESLiveUtil

+ (void)liveSetMusicSave:(NSDictionary *)itemDict{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSMutableDictionary * getSetMusicDict = [[NSMutableDictionary alloc] initWithDictionary:[userDefaults objectForKey:SETMUSICDICT]];
    [getSetMusicDict setObject:itemDict forKey:itemDict[@"bmgUrl"]];
    [userDefaults setObject:getSetMusicDict forKey:SETMUSICDICT];
    [userDefaults synchronize];
}

+ (NSDictionary *)getAllLiveMusic{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSMutableDictionary * getSetMusicDict = [userDefaults objectForKey:SETMUSICDICT];
    return getSetMusicDict;
}

+ (void)delMusic:(NSDictionary *)itemDict{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSMutableDictionary * getSetMusicDict = [[NSMutableDictionary alloc] initWithDictionary:[userDefaults objectForKey:SETMUSICDICT]];
    [getSetMusicDict removeObjectForKey:itemDict[@"bmgUrl"]];
    [userDefaults setObject:getSetMusicDict forKey:SETMUSICDICT];
}
...
+(NSString*)showTime:(NSTimeInterval) msglastTime showDetail:(BOOL)showDetail
{
    ...
    else if([nowDate timeIntervalSinceDate:msgDate] < 7 * OnedayTimeIntervalValue2)//一周内
    {
