...
@property (nonatomic,strong) SaveRoomInfoModel * roomInfoModel;
...
- (instancetype)initWithChatroom:(NIMChatroom *)room roomInfoModel:(SaveRoomInfoModel *)roomInfoModel
{
    ...
    if (self) {
        ...
        _meetingName = roomInfoModel.roomName;
...
//                            self.meetingName = meetingName;
                            self.meetingName = _roomInfoModel.roomName;
                        }
                    }
                }
                    break;
...
//                        self.meetingName = meetingName;
                ...
                case NIMChatroomEventTypeAddMute:{
                    if (content.targets.count != 0) {
                        if ([[content.targets[0] userId] isEqualToString:[NIMSDK sharedSDK].loginManager.currentAccount]) {
                            NSLog(@"被禁言了");
                            [self.delegate didUpdateMute:NTESLiveTempMuteType];
                        }
                    }
                }
                    break;
                ...
                case NIMChatroomEventTypeRemoveMute:{
                    NSLog(@"你可以发言了");
                    [self.delegate didUpdateMute:NTESLiveRelieveMuteType];
                }
                    break;
                case NIMChatroomEventTypeRoomMuted:{
                    [self.delegate didUpdateMute:NTESLiveAllMuteType];
                }
                    break;
                case NIMChatroomEventTypeRoomUnMuted:{
                    [self.delegate didUpdateMute:NTESLiveRelieveAllMuteType];
                }
                    break;
                case NIMChatroomEventTypeQueueChange:{
//                    [self.delegate didStartLive];
                }
                    break;
                case NIMChatroomEventTypeExit:{
                    if (content.targets.count != 0) {
                        NSString * userId = [content.targets[0] userId];
                        [self.delegate didStopLive:userId];
                    }
                }
                    break;
                default:
