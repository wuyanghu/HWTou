        ...
        if (![_orientationSelectView pointInside:point withEvent:nil]) {
            ...
            [UIView animateWithDuration:0.5 animations:^{
                if (_orientation == NIMVideoOrientationPortrait) {
                    _orientationSelectView.ntesBottom = self.ntesHeight + orientationSelectViewHeight;
                }
                
                else
                {
                    _orientationSelectView.ntesBottom = self.ntesWidth + orientationSelectViewHeight;
                }
...
{
    [UIView animateWithDuration:0.5 animations:^{
        _filterBar.ntesBottom = self.ntesHeight + _filterBar.ntesHeight;
    } completion:^(BOOL finished) {
...
-(void)layoutSubviews
{
    ...
    if (self.orientation == NIMVideoOrientationPortrait) {
        ...
        _cameraButton.ntesCenterX = self.ntesWidth/2;
        _cameraButton.ntesBottom = self.ntesHeight - 25;
        
        _orientationButton.ntesCenterY = _cameraButton.ntesCenterY;
        _orientationButton.ntesRight = _cameraButton.ntesLeft - 25;
        
        _beautifyButton.ntesCenterY = _cameraButton.ntesCenterY;
        _beautifyButton.ntesLeft = _cameraButton.ntesRight + 25;
        
        
        _orientationSelectView.ntesWidth = self.ntesWidth;
        _orientationSelectView.ntesHeight = orientationSelectViewHeight;
        _orientationSelectView.ntesLeft = 0;
        _orientationSelectView.ntesBottom = self.showOrientataionView? self.ntesHeight: self.ntesHeight+orientationSelectViewHeight;
        
        _filterBar.ntesWidth = self.ntesWidth;
        _filterBar.ntesHeight = _filterBar.barHeight;
        _filterBar.ntesLeft  = 0;
        _filterBar.ntesBottom = self.showFilterBar? self.ntesHeight: self.ntesHeight+_filterBar.barHeight;
    ...
    else
    {
        ...
        _cameraButton.ntesCenterX = self.ntesHeight/2;
        _cameraButton.ntesBottom = self.ntesWidth - 25;
        ...
        _orientationSelectView.ntesWidth = self.ntesHeight;
        _orientationSelectView.ntesHeight = orientationSelectViewHeight;
        _orientationSelectView.ntesLeft = 0;
        _orientationSelectView.ntesBottom = self.showOrientataionView? self.ntesWidth: self.ntesWidth+orientationSelectViewHeight;
        
        _filterBar.ntesWidth = self.ntesHeight;
        _filterBar.ntesHeight = _filterBar.barHeight;
        _filterBar.ntesLeft  = 0;
        _filterBar.ntesBottom = self.showFilterBar? self.ntesWidth: self.ntesWidth+_filterBar.barHeight;
...
- (void)showFilterView
{
    [UIView animateWithDuration:0.5 animations:^{
        if (self.orientation == NIMVideoOrientationPortrait) {
            _filterBar.ntesBottom = self.ntesHeight;
        }
        else{
            _filterBar.ntesBottom = self.ntesWidth;
        }
    } completion:^(BOOL finished) {
...
- (void)showOrientationSelectView
{
    [UIView animateWithDuration:0.5 animations:^{
        if (self.orientation == NIMVideoOrientationPortrait) {
            _orientationSelectView.ntesBottom = self.ntesHeight;
        }
        else{
            _orientationSelectView.ntesBottom = self.ntesWidth;
        }
    } completion:^(BOOL finished) {
