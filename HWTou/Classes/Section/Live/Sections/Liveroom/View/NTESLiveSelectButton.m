...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self setTitleColor:UIColorFromRGB(0x238efa) forState:UIControlStateNormal];
        [self setTitleColor:UIColorFromRGB(0x238efa) forState:UIControlStateHighlighted];
...
- (void)setCandidateObjects:(NSArray<NTESLiveSelectObject *> *)candidateObjects
{
    ...
    CGFloat width  = self.ntesSize.width;
    CGFloat height = self.ntesSize.height;
    ...
    for (NTESLiveSelectObject *object in candidateObjects) {
        ...
        button.ntesSize = CGSizeMake(width, height);
        button.hidden = YES;
...
- (void)layoutSubviews
{
    ...
    for (UIButton *button in self.subButtons) {
        ...
        {
            [button setTitleColor:UIColorFromRGB(0x238efa) forState:UIControlStateNormal];
            [button setTitleColor:UIColorFromRGB(0x238efa) forState:UIControlStateHighlighted];
        }
        else
        {
            [button setTitleColor:UIColorFromRGB(0x0) forState:UIControlStateNormal];
            [button setTitleColor:UIColorFromRGB(0x0) forState:UIControlStateHighlighted];
...
- (void)onTouch
{
    ...
    CGFloat bottom = self.superview.ntesHeight - self.ntesBottom;
    [UIView animateWithDuration:duartion animations:^{
        [self sizeToFit];
        self.ntesBottom = self.superview.ntesHeight - bottom;
        CGFloat offset = 0;
        for (UIButton *button in self.subButtons) {
            button.ntesBottom = self.ntesHeight - offset;
...
- (void)close:(UIButton *)button
{
    ...
    [UIView animateWithDuration:duartion animations:^{
        ...
        self.ntesBottom = self.superview.ntesHeight - bottom;
        for (UIButton *button in self.subButtons) {
            button.ntesBottom = self.ntesHeight;
        }
