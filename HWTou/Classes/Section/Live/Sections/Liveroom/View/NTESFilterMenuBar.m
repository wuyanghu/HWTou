...
- (void)layoutSubviews
{
    ...
    self.ntesHeight = self.barHeight;
    
    self.barTitleLab.frame = CGRectMake(10.0,
                                        20,
                                        self.barTitleLab.ntesWidth,
                                        self.barTitleLab.ntesHeight);
    
    
    
    self.menuList.frame = CGRectMake(self.barTitleLab.ntesLeft,
                                     self.barTitleLab.ntesBottom + 35,
                                     self.ntesWidth - self.barTitleLab.ntesLeft * 2,
                                     64.0);
    
    if (_showSlider) {
        self.smoothSliderLabel.frame = CGRectMake(self.menuList.ntesLeft,
                                                  self.menuList.ntesBottom + 26.0,
                                                  self.smoothSliderLabel.ntesWidth,
                                                  self.smoothSliderLabel.ntesHeight);
        
        self.smoothSlider.frame = CGRectMake(self.smoothSliderLabel.ntesRight + 4.0,
                                             0,
                                             self.ntesWidth - self.smoothSliderLabel.ntesRight - 4.0 - 12.0,
                                             32);
        self.smoothSlider.center = CGPointMake(self.smoothSlider.ntesCenterX,
                                               self.smoothSliderLabel.ntesCenterY);
        
        
        self.contrastSliderLabel.frame = CGRectMake(self.smoothSliderLabel.ntesLeft,
                                                    self.smoothSliderLabel.ntesBottom + 26.0,
                                                    self.contrastSliderLabel.ntesWidth,
                                                    self.contrastSliderLabel.ntesHeight);
        
        self.contrastSlider.frame = CGRectMake(self.smoothSlider.ntesLeft,
                                               0,
                                               self.smoothSlider.ntesWidth,
                                               self.smoothSlider.ntesHeight);
        self.contrastSlider.center = CGPointMake(self.contrastSlider.ntesCenterX,
                                                 self.contrastSliderLabel.ntesCenterY);

    }
    
    
    self.topLine.frame = CGRectMake(0, self.barTitleLab.ntesBottom + 20, self.ntesWidth, 0.5);
    
    self.bottomLine.frame = CGRectMake(0, _showSlider? self.contrastSlider.ntesBottom + 20:self.menuList.ntesBottom  + 20, self.ntesWidth, 0.5);
    
    self.verticalLine.frame = CGRectMake(self.ntesWidth * 0.5, _showSlider? self.contrastSlider.ntesBottom + 20 :self.menuList.ntesBottom  + 20, 0.5, _showSlider? self.ntesHeight - self.contrastSlider.ntesBottom - 20: self.ntesHeight - self.menuList.ntesBottom - 20);

    self.cancelButton.frame = CGRectMake(0, _showSlider? self.contrastSlider.ntesBottom+ 20:self.menuList.ntesBottom  + 20, self.ntesWidth * 0.5,_showSlider? self.ntesHeight - self.contrastSlider.ntesBottom - 20: self.ntesHeight - self.menuList.ntesBottom - 20);

    self.confirmButton.frame = CGRectMake(self.ntesWidth * 0.5, _showSlider? self.contrastSlider.ntesBottom+ 20:self.menuList.ntesBottom  + 20, self.ntesWidth * 0.5,_showSlider? self.ntesHeight - self.contrastSlider.ntesBottom - 20: self.ntesHeight - self.menuList.ntesBottom - 20);
...
{
    return CGSizeMake(self.menuList.ntesHeight - 15, self.menuList.ntesHeight);
}
...
- (UIView *)topLine
{
    if (!_topLine) {
        ...
        _topLine.backgroundColor = UIColorFromRGB(0x545454);
    }
    return _topLine;
}
...
- (UIView *)bottomLine
{
    if (!_bottomLine) {
        ...
        _bottomLine.backgroundColor = UIColorFromRGB(0x545454);
    }
    return _bottomLine;
}
