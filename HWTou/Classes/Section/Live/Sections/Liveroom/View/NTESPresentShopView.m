...
- (NTESPresentShopBar *)bar
{
    if (!_bar) {
        ...
        _bar = [[NTESPresentShopBar alloc] initWithFrame:CGRectMake(0, 0, self.ntesWidth, height + 40)];
        _bar.delegate = self;
    }
    return _bar;
}
...
- (UIButton *)sendButton
{
    if (!_sendButton) {
        ...
        _sendButton.ntesSize = CGSizeMake(57.f, 26.f);
        _sendButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleLeftMargin;
        _sendButton.ntesRight =  self.ntesWidth  - 8.f;
        _sendButton.ntesBottom = self.ntesHeight - 5.f;
