...
- (void)setup
{
    ...
    _stopBypassButton.ntesSize = CGSizeMake(44, 44);
...
- (void)layoutSubviews
{
    ...
    self.exitConfirmView.ntesWidth = self.ntesWidth;
    self.exitConfirmView.ntesHeight = self.ntesHeight;
    self.exitConfirmView.ntesBottom = self.ntesHeight;
    self.exitConfirmView.ntesRight = self.ntesWidth;
    self.loadingView.frame = self.bounds;
    self.endView.frame = self.bounds;
    
    self.stopBypassButton.ntesRight = self.ntesWidth;
}
...
{
    [super layoutSubviews];
    _topSeperator.ntesSize    = CGSizeMake(self.ntesWidth, 0.5f);
    _bottomSeperator.ntesSize = CGSizeMake(self.ntesWidth, 0.5f);
    ...
    if ([NTESLiveManager sharedInstance].orientation==NIMVideoOrientationLandscapeRight&&[NTESLiveManager sharedInstance].type == NTESLiveTypeVideo) {
        ...
        _titleLabel.ntesTop = 0;
        _titleLabel.ntesWidth = self.ntesWidth;
        _titleLabel.ntesHeight = self.ntesHeight/3;
        _titleLabel.ntesCenterX = self.ntesWidth * .5f;

        _cancelButton.ntesWidth = self.ntesWidth;
        _cancelButton.ntesHeight = self.ntesHeight/3;
        _cancelButton.ntesBottom  = self.ntesHeight;

        _confirmButton.ntesWidth = self.ntesWidth;
        _confirmButton.ntesHeight = self.ntesHeight/3;
        _confirmButton.ntesBottom = _cancelButton.ntesTop;

        _topSeperator.ntesBottom  = _confirmButton.ntesTop;
        _bottomSeperator.ntesBottom = _cancelButton.ntesTop;

    }
    else
    {
        _titleLabel.ntesSize = CGSizeMake(78, 32);
        _titleLabel.ntesTop = top;
        _titleLabel.ntesCenterX = self.ntesWidth * .5f;
        _cancelButton.ntesSize = CGSizeMake(100, 41);
        _cancelButton.ntesBottom  = self.ntesHeight;
        _confirmButton.ntesSize = CGSizeMake(100, 41);
        _confirmButton.ntesBottom = _cancelButton.ntesTop;
        _topSeperator.ntesBottom  = _confirmButton.ntesTop;
        _bottomSeperator.ntesBottom = _cancelButton.ntesTop;
    }
...
- (void)layoutSubviews
{
    ...
    self.avatar.ntesTop = avatarTop;
    self.avatar.ntesCenterX = self.ntesWidth * .5f;
    self.nickLabel.ntesTop = self.avatar.ntesBottom + nickAndAvatarSpacing;
    self.nickLabel.ntesCenterX = self.ntesWidth * .5f;
    self.statusLabel.ntesTop = self.nickLabel.ntesBottom + nickAndStatusSpacing;
    self.statusLabel.ntesCenterX = self.ntesWidth * .5f;
}
