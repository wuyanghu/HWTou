...
- (void)setupSubviews {
    ...
    if (!_isSelf){
        ...
        shieldBtn.frame = CGRectMake(204, 219, 50, 14);
        [shieldBtn setBackgroundImage:[UIImage imageNamed:@"Shield"] forState:UIControlStateNormal];
        shieldBtn.layer.cornerRadius = 15.f;
        [shieldBtn addTarget:self action:@selector(shieldBtnAction:) forControlEvents:UIControlEventTouchUpInside];
        [bgView addSubview:shieldBtn];
        
        UIButton *reportBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        reportBtn.frame = CGRectMake(266, 219, 50, 14);
        [reportBtn setBackgroundImage:[UIImage imageNamed:@"Report"] forState:UIControlStateNormal];
        reportBtn.layer.cornerRadius = 15.f;
        ...
        if (self.model.isShield == 1) {
            shieldBtn.enabled = NO;
        }
    }
...
- (void)shieldBtnAction:(UIButton *)button{
    NSLog(@"屏蔽");
    [button setEnabled:NO];
    ShieldSomeOneParam * param = [ShieldSomeOneParam new];
    param.shieldId = self.userId;
    [PersonHomeRequest shieldSomeOne:param Success:^(BaseResponse *response) {
        if (response.status == 200) {
            [HUDProgressTool showSuccessWithText:@"已屏蔽"];
        }
...
- (void)reportBtnAction:(UIButton *)button{
    NSLog(@"举报");
    [button setEnabled:NO];
    ReportSomeOneParam * param = [ReportSomeOneParam new];
    param.reportId = self.userId;
    [PersonHomeRequest reportSomeOne:param Success:^(BaseResponse *response) {
        if (response.status == 200) {
            [HUDProgressTool showSuccessWithText:@"举报成功"];
        }
    } failure:^(NSError *error) {
