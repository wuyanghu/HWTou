...
- (void)setUpSubViews
{
    ...
    for (NSNumber *type in self.types) {
        ...
        CGFloat width   = self.ntesWidth - 2 * spacing;
...
- (void)layoutSubviews
{
    ...
    _bar.ntesBottom = self.ntesHeight;
    
    CGFloat titleTop = 10.f;
    self.titleLabel.ntesTop = titleTop;
    self.titleLabel.ntesCenterX = self.bar.ntesWidth * .5f;
    
    CGFloat height = self.bar.ntesHeight - 2 *titleTop - self.titleLabel.ntesHeight;
    
    CGFloat buttonHeight = 0;
    for (UIButton *button in self.buttons) {
        buttonHeight += button.ntesHeight;
    }
    CGFloat padding = (height - buttonHeight)/(self.buttons.count + 1);
    
    CGFloat top = self.titleLabel.ntesBottom + titleTop;
    for (UIButton *button in self.buttons) {
        top += padding;
        button.ntesTop = top;
        button.ntesCenterX = self.bar.ntesWidth * .5f;
        top += button.ntesHeight;
    }
}
