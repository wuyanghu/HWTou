...
- (void)layoutSubviews
{
    ...
    if (_selectIcon)
    {
        ...
        self.label.frame = CGRectMake(0,
                                      self.ntesHeight - self.label.ntesHeight,
                                      self.label.ntesWidth,
                                      self.label.ntesHeight);
        self.imageView.center = CGPointMake(self.ntesWidth/2, self.imageView.ntesCenterY);
    }
    else
    {
        
        self.imageView.frame = CGRectMake(0, 0, imgWidth, imgWidth);
        self.imageView.center = CGPointMake(self.ntesWidth/2, self.ntesHeight/2 - 12.0);
        self.label.frame = CGRectMake(0,
                                      self.imageView.ntesBottom + 12.0,
                                      self.label.ntesWidth,
                                      self.label.ntesHeight);
        
    }
    self.label.ntesCenterX = self.imageView.ntesCenterX;
}
