...
@property(nonatomic, strong) NTESessionMessageDataProvider *dataPorvider;
...
- (instancetype)initWithSession:(NIMSession *)session andFirstMsg:(NIMMessage *)msg {
    ...
    if (self = [super init]) {
        _session = session;
        _firstMsg = msg;
        _dataPorvider = [[NTESessionMessageDataProvider alloc] initWithSession:self.session andFirstMsg:_firstMsg];
    }
    return self;
}
...
- (id<NIMKitMessageProvider>)messageDataProvider {
    return self.dataPorvider;
}

- (NSTimeInterval)historyTimeTag {
    return self.firstMsg.timestamp;
}
