...
{
    CGFloat width  = (collectionView.ntesWidth - self.minimumInteritemSpacing * (self.itemCountPerLine - 1)) / self.itemCountPerLine;
...
{
    return CGSizeMake(collectionView.ntesWidth, 45);
}
...
- (void)layoutSubviews
{
    ...
    CGFloat right = 5.f;
    CGFloat botttom = 5.f;
    self.durationLabel.ntesRight = self.ntesWidth - right;
    self.durationLabel.ntesBottom = self.ntesHeight - botttom;
}
...
{
    [super layoutSubviews];
    self.titleLabel.ntesLeft = 10.f;
    self.titleLabel.ntesCenterY = self.ntesHeight * .5f;
}
