...
- (void)viewDidLoad {
    ...
    self.searchController.searchBar.ntesHeight = 44.f; // iOS8 下 searchBar 默认高度为 0，需要预设一个高度进去
...
{
    CGFloat resultInsetTop = self.searchController.searchBar.ntesHeight + [UIApplication sharedApplication].statusBarFrame.size.height;
    UIEdgeInsets resultInsets = {resultInsetTop, 0, 0, 0};
    self.searchResultController.tableView.contentInset = resultInsets;
}

- (void)viewDidLayoutSubviews{
    self.noResultTip.ntesCenterX = self.tableView.ntesWidth * .5f;
    self.noResultTip.ntesCenterY = self.tableView.ntesHeight * .5f;
}
...
- (UIView *)tableFooterView{
    ...
    lable.ntesCenterX = btn.ntesWidth  * .5f;
    lable.ntesCenterY = btn.ntesHeight * .5f;
