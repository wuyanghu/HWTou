...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _micButton.ntesSize = CGSizeMake(35, 35);
...
- (void)layoutSubviews
{
    ...
    self.avatarImageView.ntesLeft = avatarLeft;
    self.avatarImageView.ntesCenterY = self.ntesHeight * .5f;
    CGFloat nameAndAvatarSpacing = 5.f;
    self.nameLabel.ntesLeft = self.avatarImageView.ntesRight + nameAndAvatarSpacing;
    self.nameLabel.ntesCenterY = self.ntesHeight * .5f;
    CGFloat micButtonRight = 10.f;
    self.micButton.ntesRight   = self.ntesWidth - micButtonRight;
    self.micButton.ntesCenterY = self.ntesHeight * .5f;
}
