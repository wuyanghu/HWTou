...
- (void)refreshWithDefaultAvatar:(NSString *)user
{
    ...
    self.nameLabel.ntesWidth = self.ntesWidth;
}
...
- (void)layoutSubviews
{
    ...
    self.tipLabel.ntesCenterX = self.contentView.ntesWidth  * .5f;
    self.tipLabel.ntesCenterY = self.contentView.ntesHeight * .5f;
    self.connectingImageView.ntesCenterX = self.contentView.ntesWidth  * .5f;
    self.connectingImageView.ntesCenterY = self.contentView.ntesHeight * .5f;
    self.nameLabel.ntesBottom = self.contentView.ntesHeight;
    self.nameLabel.ntesCenterX = self.contentView.ntesWidth * .5f;
    self.progressView.ntesBottom = self.contentView.ntesHeight;
}
...
{
    if (!_progressView) {
        _progressView = [[UIProgressView alloc] initWithFrame:CGRectMake(0, 0, self.contentView.ntesWidth, 1)];
...
- (void)layoutSubviews
{
    [super layoutSubviews];
    CGFloat thin = .5f;
    CGFloat spacing = self.ntesWidth / (self.divid + 1);
    CGFloat left = spacing;
    for (NSInteger index = 0; index < self.divid; index++)
    {
        UIView *sep = self.seps[index];
        sep.frame = CGRectMake(left, 0, thin, self.ntesHeight);
        left += spacing;
    }
    ...
    {
        UIView *sep = self.seps[index];
        sep.frame = CGRectMake(0, top, self.ntesWidth, thin);
        top += spacing;
    }
}
