...
- (void)addControl{
    ...
    self.numLabel.layer.borderWidth = Single_Line_Width;
...
- (void)layoutSubviews{
    ...
    if (self.frame.size.width>=self.frame.size.height) {
        self.subtractBtn.frame = CGRectMake(0, 0, self.bounds.size.height * 1.3, self.bounds.size.height);
        self.addBtn.frame = CGRectMake(self.bounds.size.width - self.bounds.size.height * 1.3, 0, self.bounds.size.height * 1.3, self.bounds.size.height);
        self.numLabel.frame = CGRectMake(self.subtractBtn.frame.size.width, 0, self.bounds.size.width - 2*(self.subtractBtn.frame.size.width), self.bounds.size.height);
    }else{
        ...
        self.numLabel.frame = CGRectMake(0, self.subtractBtn.frame.size.height, self.bounds.size.width, self.bounds.size.height - 2*(self.subtractBtn.frame.size.height));
    }
...
- (void)btnClick:(UIButton *)btn{
    ...
    if (btn.tag == 0) {
        ...
        if      (self.numLabel.text.intValue==self.minNum)              self.numLabel.text = self.numLabel.text;
        else if (self.numLabel.text.intValue-self.unitNum<self.minNum)  self.numLabel.text = [NSString stringWithFormat:@"%d",self.minNum];
        else    self.numLabel.text = [NSString stringWithFormat:@"%d",self.numLabel.text.intValue - self.unitNum];
        ...
        if      (self.numLabel.text.intValue==self.maxNum)             self.numLabel.text = self.numLabel.text;
        else if (self.numLabel.text.intValue+self.unitNum>self.maxNum) self.numLabel.text = [NSString stringWithFormat:@"%d",self.maxNum];
        else    self.numLabel.text = [NSString stringWithFormat:@"%d",self.numLabel.text.intValue + self.unitNum];
